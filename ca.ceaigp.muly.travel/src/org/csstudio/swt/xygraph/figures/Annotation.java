...
public class Annotation extends Figure implements IAxisListener, IDataProviderListener
{
	...
	protected void paintFigure(Graphics graphics)
	{
		...
		if (trace != null && currentSnappedSample == null && !pointerDragged) updateToDefaultPosition();
		...
		else tempColor = annotationColor;
		...
		Rectangle infoBounds = new Rectangle((int) (currentPosition.x + x0 - size.width / 2.0), (int) (currentPosition.y + y0 - size.height / 2.0), size.width,
		        size.height);
		...
		switch (cursorLineStyle)
		{
			case NONE:
				// left
				graphics.drawLine(currentPosition.x - POINT_SIZE / 2, currentPosition.y, currentPosition.x - POINT_SIZE / 2 - CURSOR_LINE_LENGTH,
				        currentPosition.y);
				// right
				graphics.drawLine(currentPosition.x + POINT_SIZE / 2, currentPosition.y, currentPosition.x + POINT_SIZE / 2 + CURSOR_LINE_LENGTH,
				        currentPosition.y);
				// up
				graphics.drawLine(currentPosition.x, currentPosition.y - POINT_SIZE / 2, currentPosition.x, currentPosition.y - POINT_SIZE / 2
				        - CURSOR_LINE_LENGTH);
				// down
				graphics.drawLine(currentPosition.x, currentPosition.y + POINT_SIZE / 2, currentPosition.x, currentPosition.y + POINT_SIZE / 2
				        + CURSOR_LINE_LENGTH);
			...
			case LEFT_RIGHT:
				// left
				graphics.drawLine(currentPosition.x - POINT_SIZE / 2, currentPosition.y, xAxis.getValuePosition(xAxis.getRange().getLower(), false),
				        currentPosition.y);
				// right
				graphics.drawLine(currentPosition.x + POINT_SIZE / 2, currentPosition.y, xAxis.getValuePosition(xAxis.getRange().getUpper(), false),
				        currentPosition.y);
				if (cursorLineStyle != CursorLineStyle.FOUR_DIRECTIONS) break;
			case UP_DOWN:
				// up
	...
	public void updateX0Y0Fromdxdy(Dimension size)
	{
		if (!knowX0Y0)
		{
			...
			if (dy != 0)
			{
				...
				y0 = dy - h / 2.0;
				x0 = (dx / dy) * y0;
				if (new Range(0, x0).inRange(dx) && new Range(0, y0).inRange(dy) && new Range(x0 - w / 2.0, x0 + w / 2.0).inRange(dx)) return;
			...
			if (dx != 0)
			{
				x0 = dx + w / 2.0;
				y0 = (dy / dx) * x0;
				if (new Range(0, x0).inRange(dx) && new Range(0, y0).inRange(dy) && new Range(y0 - h / 2.0, y0 + h / 2.0).inRange(dy)) return;
	...
	private void updateToDefaultPosition()
	{
		if (trace != null && trace.getHotSampleList().size() > 0)
		{
			...
			currentPosition = new Point(xAxis.getValuePosition(currentSnappedSample.getXValue(), false), yAxis.getValuePosition(
			        currentSnappedSample.getXValue(), false));
		...
		else
		{
			...
			if (xAxis.isLogScaleEnabled()) xValue = Math.pow(10, (Math.log10(xAxis.getRange().getLower()) + Math.log10(xAxis.getRange().getUpper())) / 2);
			else xValue = (xAxis.getRange().getLower() + xAxis.getRange().getUpper()) / 2;
			if (yAxis.isLogScaleEnabled()) yValue = Math.pow(10, (Math.log10(yAxis.getRange().getLower()) + Math.log10(yAxis.getRange().getUpper())) / 2);
			else yValue = (yAxis.getRange().getLower() + yAxis.getRange().getUpper()) / 2;
	...
	{
		String info = "";
		if (showName) info = name;
		if (showSampleInfo && currentSnappedSample != null && !currentSnappedSample.getInfo().equals("")) info += "\n" + currentSnappedSample.getInfo();
		if (showPosition) info += "\n" + "(" + xAxis.format(xValue) + ", " + (Double.isNaN(yValue) ? "NaN" : yAxis.format(yValue)) + ")";
	...
	public void setFont(Font f)
	{
		...
		if (f != null) this.fontData = getFont().getFontData()[0];
	}
	...
	public void setTrace(Trace trace)
	{
		...
		if (!isFree() && this.trace != trace) this.trace.getDataProvider().removeDataProviderListener(this);
	...
	public void setAnnotationColor(Color annotationColor)
	{
		...
		if (annotationColor != null) this.annotationColorRGB = annotationColor.getRGB();
	...
	{
		if (!trace.getHotSampleList().contains(currentSnappedSample)) updateToDefaultPosition();
		else
		{
			this.currentSnappedSample = currentSnappedSample;
			Point newPosition = new Point(xAxis.getValuePosition(currentSnappedSample.getXValue(), false), yAxis.getValuePosition(
			        currentSnappedSample.getYValue(), false));
			xValue = currentSnappedSample.getXValue();
			yValue = currentSnappedSample.getYValue();
			if (Double.isNaN(currentSnappedSample.getXPlusError())) yValue = Double.NaN;
	...
	class Pointer extends Figure
	{
		...
		class PointerDragger extends MouseMotionListener.Stub implements MouseListener
		{
			...
			public void mouseDragged(MouseEvent me)
			{
				...
				{ // snap to trace
					// double tempX = xAxis.getPositionValue(me.getLocation().x,
					// false);
					// double tempY = yAxis.getPositionValue(me.getLocation().y,
					// false);
					...
					if (tempSample != null && currentSnappedSample != tempSample) setCurrentSnappedSample(tempSample,
			...
			public void mousePressed(MouseEvent me)
			{
				...
				if (isFree()) command.setBeforeMovePosition(currentPosition);
				else command.setBeforeMoveSnappedSample(currentSnappedSample);
			...
			public void mouseReleased(MouseEvent me)
			{
				if (command != null)
				{
					if (isFree()) command.setAfterMovePosition(currentPosition);
					else command.setAfterMoveSnappedSample(currentSnappedSample);
