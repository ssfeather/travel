...
public abstract class AbstractScale extends Figure
{
	...
	public String format(Object obj, boolean minOrMaxDate)
	{
		...
		if (isDateEnabled())
		{
			if (autoFormat || formatPattern == null || formatPattern.equals("") || formatPattern.equals(default_decimal_format)
			        || formatPattern.equals(DEFAULT_ENGINEERING_FORMAT))
			{
				formatPattern = DEFAULT_DATE_FORMAT;
				double length = Math.abs(max - min);
				if (length <= 5000 || timeUnit == Calendar.MILLISECOND)
				{ // less than five second
					formatPattern = "ss.SSS";//$NON-NLS-1$
				}
				else if (length <= 1800000d || timeUnit == Calendar.SECOND)
				{ // less than 30 min
			...
			{
				if (Math.abs(max - min) < 5000) return new SimpleDateFormat("yyyy-MM-dd\nHH:mm:ss.SSS").format(obj); //$NON-NLS-1$
	...
	public void setRange(double lower, double upper)
	{
		...
		{
			upper = lower + 1;
			if (Double.isInfinite(upper)) throw new IllegalArgumentException("Illegal range: lower=" + lower + ", upper=" + upper);
		}
		...
		{
			if (Math.abs(max - min) > 0.1) default_decimal_format = "############.##";
			else
			{
		...
		if (formatPattern.equals(default_decimal_format) || formatPattern.equals(DEFAULT_ENGINEERING_FORMAT))
		{
			if ((max != 0 && Math.abs(Math.log10(Math.abs(max))) >= ENGINEERING_LIMIT)
			        || (min != 0 && Math.abs(Math.log10(Math.abs(min))) >= ENGINEERING_LIMIT)) formatPattern = DEFAULT_ENGINEERING_FORMAT;
			else formatPattern = default_decimal_format;
			autoFormat = true;
		}
		range = new Range(min, max);
		setDirty(true);
